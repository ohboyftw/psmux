...
pub fn toggle_zoom(app: &mut AppState) {
    ...
    } else if let Some(saved) = app.windows[app.active_idx].zoom_saved.take() {
        let win = &mut app.windows[app.active_idx];
        for (p, sz) in saved.into_iter() {
            if let Some(Node::Split { sizes, .. }) = get_split_mut(&mut win.root, &p) {
                *sizes = sz;
...
pub fn respawn_active_pane(
    ...
    // With -k, kill the running process first and respawn.
    {
        ...
        if let Some(pane) = crate::tree::active_pane(&win.root, &win.active_path) {
            if !pane.dead && !kill {
                return Err(io::Error::other("pane still active"));
            }
        }
    }
    if kill {
    ...
    pane.child_pid = crate::platform::mouse_inject::get_child_pid(&*child);
    pane.child = child;
    pane.term = term;
