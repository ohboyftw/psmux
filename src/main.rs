...
fn run_main() -> io::Result<()> {
    ...
    match cmd {
                            ...
                            if let Ok(port_str) = std::fs::read_to_string(&path) {
                                if let Ok(port) = port_str.trim().parse::<u16>() {
                                    ...
                                    if let Ok(mut stream) = std::net::TcpStream::connect_timeout(
                                        ...
                                        let _ = writeln!(stream, "AUTH {}", sess_key);
                                        let _ = stream.flush();
                                    ...
                                    if let Ok(_p) = port_str.trim().parse::<u16>() {
                                        ...
                                        if let Ok(mut s) = std::net::TcpStream::connect_timeout(
                                            ...
                                            if !line.trim().is_empty()
                                                && line.trim() != "ERROR: Authentication required"
                                            {
        ...
        "new-session" | "new" => {
                    ...
                    while k < chars.len() {
                        ...
                        match c {
                            's' => {
                                i += 1;
                                if i < cmd_args.len() {
                                    session_name = Some(cmd_args[i].to_string());
                                }
                                break;
                            }
                            'n' => {
                                i += 1;
                                if i < cmd_args.len() {
                                    window_name = Some(cmd_args[i].to_string());
                                }
                                break;
                            }
                            'F' => {
                            ...
                            'x' => {
                                i += 1;
                                if i < cmd_args.len() {
                                    init_width = cmd_args[i].parse::<u16>().ok();
                                }
                                break;
                            }
                            'y' => {
                                i += 1;
                                if i < cmd_args.len() {
                                    init_height = cmd_args[i].parse::<u16>().ok();
                                }
                                break;
                            }
                            'e' | 'f' | 't' => {
            ...
            }
            {
                ...
                if !server_alive {
                    let _ = std::fs::remove_file(&port_path);
                    eprintln!(
                        "psmux: session '{}' exited immediately (check shell command)",
                        name
                    );
                    std::process::exit(1);
                }
            ...
            if let Ok(port_str) = std::fs::read_to_string(&path) {
                if let Ok(port) = port_str.trim().parse::<u16>() {
                    ...
                    if let Ok(mut s) = std::net::TcpStream::connect_timeout(
                        ...
                        let _ = writeln!(s, "AUTH {}", session_key);
                        let _ = writeln!(s, "session-info");
                        let _ = s.flush();
                        let mut buf = [0u8; 256];
        ...
        "start-server" | "start" | "warmup" => {
            ...
            // "warmup" is a legacy alias kept for backward compatibility.
            return Ok(());
        }
