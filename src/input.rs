    ...
    match app.mode {
        Mode::Passthrough => {
            ...
            let is_prefix = (key.code, key.modifiers) == app.prefix_key
                ...
                || (app.prefix2_key == Some((key.code, key.modifiers)));
            if is_prefix {
            ...
            match key.code {
                ...
                KeyCode::Enter => {
                    if let Mode::RenamePrompt { input } = &mut app.mode {
                        app.windows[app.active_idx].name = input.clone();
                        app.mode = Mode::Passthrough;
                    }
                }
                KeyCode::Backspace => {
        ...
        if matches!(key.code, KeyCode::Enter) && !key.modifiers.is_empty() {
            ...
            if app.sync_input {
                ...
                if let Some(active) = active_pane_mut(&mut win.root, &win.active_path) {
                    if !active.dead && try_inject(active) {
                        return Ok(());
...
fn forward_mouse_to_pane_ex(
    ...
    crate::window_ops::inject_mouse_combined(pane, col, row, vt_button, press, 0, 0, "client");
}
    ...
    match me.kind {
        ...
        MouseEventKind::Down(MouseButton::Right) => {
            ...
            let wants_mouse = active_pane(&win.root, &win.active_path)
                .is_some_and(crate::window_ops::pane_wants_mouse);
            if wants_mouse {
                if let Some(area) = active_area {
                    if let Some(active) = active_pane_mut(&mut win.root, &win.active_path) {
                        if crate::window_ops::pane_wants_mouse(active) {
                ...
                if let Some(area) = active_area {
                    if let Some(active) = active_pane_mut(&mut win.root, &win.active_path) {
                        ...
                        forward_mouse_to_pane_ex(
                            ...
                            64,
                            false,
                        ); // SGR button 64 = scroll-up, release
                    }
                }
            } else {
                ...
                if let Some(area) = active_area {
                    if let Some(active) = active_pane_mut(&mut win.root, &win.active_path) {
                        ...
                        forward_mouse_to_pane_ex(
                            ...
                            65,
                            true,
                        ); // SGR button 65 = scroll-down, press
                        forward_mouse_to_pane_ex(
                            active,
                            area,
                            me.column,
                            ...
                            65,
                            false,
                        ); // SGR button 65 = scroll-down, release
                    }
                }
            }
        }
...
pub fn send_key_to_active(app: &mut AppState, k: &str) -> io::Result<()> {
    ...
    if let Some(p) = active_pane_mut(&mut win.root, &win.active_path) {
        match k {
            "enter" => {
                let _ = write!(p.writer, "\r");
            }
            "tab" => {
                let _ = write!(p.writer, "\t");
            }
            "btab" | "backtab" => {
                let _ = write!(p.writer, "\x1b[Z");
            }
            "backspace" => {
                let _ = p.writer.write_all(&[0x7F]);
            }
            "delete" => {
                let _ = write!(p.writer, "\x1b[3~");
            }
            "esc" => {
                let _ = write!(p.writer, "\x1b");
            }
            "left" => {
                let _ = write!(p.writer, "\x1b[D");
            }
            "right" => {
                let _ = write!(p.writer, "\x1b[C");
            }
            "up" => {
                let _ = write!(p.writer, "\x1b[A");
            }
            "down" => {
                let _ = write!(p.writer, "\x1b[B");
            }
            "pageup" => {
                let _ = write!(p.writer, "\x1b[5~");
            }
            "pagedown" => {
                let _ = write!(p.writer, "\x1b[6~");
            }
            "home" => {
                let _ = write!(p.writer, "\x1b[H");
            }
            "end" => {
                let _ = write!(p.writer, "\x1b[F");
            }
            "insert" => {
                let _ = write!(p.writer, "\x1b[2~");
            }
            "space" => {
                let _ = write!(p.writer, " ");
            }
