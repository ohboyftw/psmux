...
pub const PREFIX_DEFAULTS: &[(&str, &str)] = &[
    ...
    ("]", "paste-buffer"),
    ("=", "choose-buffer"),
    ("f", "hints (quick-select URLs/paths/hashes)"),
    // ── Misc ──
    (":", "command-prompt"),
    ("?", "list-keys"),
    ("i", "display-message"),
...
pub fn hooks_lines() -> Vec<String> {
    vec![
        String::new(),
        "── hooks (set-hook) ───────────────────────────────────────".into(),
        "  after-new-session     after-new-window      after-kill-pane".into(),
        "  after-split-window    after-select-window    after-select-pane".into(),
        "  after-resize-pane     after-rename-window    after-rename-session".into(),
        "  after-select-layout   after-copy-mode        after-set-option".into(),
        "  after-bind-key        after-unbind-key       after-source".into(),
        "  after-swap-pane       after-swap-window      client-attached".into(),
...
pub fn mouse_lines() -> Vec<String> {
    vec![
        String::new(),
        "── mouse bindings (when mouse is on) ──────────────────────".into(),
        "  Left click status tab    switch to clicked window".into(),
        "  Left click pane          focus pane (+ forward to child)".into(),
        "  Left click border        begin drag-resize".into(),
        "  Left drag border         resize split interactively".into(),
        "  Scroll up/down           forward wheel to child (or copy mode scroll)".into(),
    ]
}
