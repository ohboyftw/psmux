...
/// Send a control message and block indefinitely until the server responds.
/// Unlike `send_control_with_response`, this has no read timeout — the
/// connection stays open until the server writes a response and closes it.
/// Used for long-blocking commands like `wait-pane`.
pub fn send_control_with_response_blocking(line: String) -> io::Result<String> {
    let home = env::var("USERPROFILE")
        .or_else(|_| env::var("HOME"))
        .unwrap_or_default();
    let mut target = env::var("PSMUX_TARGET_SESSION")
    ...
    let mut stream = std::net::TcpStream::connect(&addr)?;
    let _ = stream.set_nodelay(true);
    // No read timeout — block until server responds
